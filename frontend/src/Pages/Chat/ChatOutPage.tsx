...
function transformToMailContact(data: SellerData): MailContact {
  return {
    ...
    avatar: data.seller_name.charAt(0),
    unread: data.unread,
  };
}

export function ChatOutPage({ isAdmin, onLogout }: { isAdmin : boolean; onLogout: () => void; }) {
...
const [mailThreads, setMailThreads] = useState<Record<number, Array<{ 
    ...
    useEffect(() => {
        ...
        const checkMail = async () => {
            try  {
                const res = await fetch_with_auth.get("/get_out_conversations/");
